...
LOG_FILE="${SUPPORT_DIR}/logs/JAMFRetrieveEmail.log"
...
function create_display_list ()
{
    ...
    echo $UserList | jq -r '.users[].name' | while IFS= read -r line; do
...
function JAMF_retrieve_json_data ()
{    
    ...
    echo $(/usr/bin/curl -s --header "Authorization: Bearer ${api_token}" -H "Accept: application/json" "${jamfpro_url}${1}" )
}

function JAMF_retrieve_json_data_details ()
{    
    # PURPOSE: Extract the summary of the JAMF conmand results
    # RETURN: XML contents of command
    ...
    #   api_token - base64 hex code of your bearer token
    #   jamppro_url - the URL of your JAMF server   
    jsonBlob=$(/usr/bin/curl -s --header "Authorization: Bearer ${api_token}" -H "Accept: application/json" "${jamfpro_url}${1}")
}
...
function extract_user_details ()
{
    ...
    # EXPECTED: jsonBlob should be globally defined
    declare managed && managed=true

	[[ -z "${1}" ]] && return 0

    JAMF_retrieve_json_data_details "JSSResource/users/id/$1"
    
    userShortName=$(extract_json_data $jsonBlob "name")
    userFullName=$(extract_json_data $jsonBlob  "full_name")
    userEmail=$(extract_json_data $jsonBlob "email_address")
    userPosition=$(extract_json_data $jsonBlob "position")
    ...
    if [[ -z $userFullName ]]; then
        ...
        if [[ $ssOnlyManagedUsers == "true" ]]; then
            ...
            managed=$(extract_assigned_systems ${jsonBlob})
        fi
...
UserList=$(JAMF_retrieve_json_data "JSSResource/users")
UserIDs=($(echo $UserList | jq -r '.users[].id'))
userCount=${#UserIDs[@]}
