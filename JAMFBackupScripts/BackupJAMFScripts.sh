...
JSON_DIALOG_BLOB=$(mktemp /var/tmp/JAMFScriptBackup.XXXXX)
...
SUPPORT_DIR="/Library/Application Support/GiantEagle"
LOG_FILE="${SUPPORT_DIR}/logs/JAMFScriptBackup.log"

# Display items (banner / icon)

SD_WINDOW_TITLE="JAMF Script Backup"
...
function welcomemsg ()
{
    message="This script will extract all of the system scripts from your JAMF server and store them in local folder structure.  Please select a destination folder location below:"
...
{
    construct_dialog_header_settings "The following JAMF scripts will be backed up" > "${JSON_DIALOG_BLOB}"
    create_listitem_message_body "" "" "" "" "first"

    echo $ScriptList | xmllint --xpath '//name' - | while IFS= read -r line; do
...
function extract_script_details ()
{
    ...
    if [[ -z $scriptContents ]]; then
        update_display_list "Update" "" "${formatted_scriptName}" "" "fail" "Not exported!"
        failedMigration+=("${formatted_scriptName}")
    else
        update_display_list "Update" "" "${formatted_scriptName}" "" "wait" "Working..."
        # Store the script in the destination folder
        echo "${scriptContents}" > "${ssStoragePath}/${formatted_scriptName}.sh"
